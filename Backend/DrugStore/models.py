...
class Accounts(models.Model):
    ...
    userid = models.CharField(max_length=100, unique=True, default=uuid.uuid4, editable=False)
...
class Purchases(models.Model):
    ...
    account = models.ForeignKey('Accounts', on_delete=models.CASCADE, related_name='purchases')
    product = models.ForeignKey('Products', on_delete=models.CASCADE, related_name='purchases')
...
class UserViewHistory(models.Model):
    user = models.ForeignKey(Accounts, on_delete=models.CASCADE, related_name='view_history')
    product = models.ForeignKey(Products, on_delete=models.CASCADE, related_name='view_history_entries')
    ...
    class Meta:
        ...
        unique_together = [['user', 'product']] # Ensure a user can only have one view entry per product
        
    def __str__(self):
        # Format viewed_at for better readability in admin or logs
        formatted_time = self.viewed_at.strftime("%Y-%m-%d %H:%M:%S") if self.viewed_at else "N/A"
...
class CartItem(models.Model):
    user = models.ForeignKey(Accounts, on_delete=models.CASCADE, related_name='cart_items')
    product = models.ForeignKey(Products, on_delete=models.CASCADE, related_name='cart_item_entries')
    quantity = models.PositiveIntegerField(default=1)
    added_at = models.DateTimeField(auto_now_add=True)  # Time item was added to cart
    updated_at = models.DateTimeField(auto_now=True)    # Time item was last updated (e.g., quantity change)
